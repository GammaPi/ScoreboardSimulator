...
from Simulator.FunctionUnit import FunctionUnit
...
class ControlUnit:
    ...
    def __init__(self, file_path):
        '''
          Control Unit for simulator
          :param file_path Path to a input file
        '''
        self.pc = 0
        self.current_cycle = 1

        self.decoder = Assembler(file_path=file_path)
    ...
    def can_issue(self, instruction, function_unit):
        ...
        return instruction != None and instruction.fu == function_unit.type and (not function_unit.busy) and (
            not self.reg_status[int(instruction.dst[1:])])
